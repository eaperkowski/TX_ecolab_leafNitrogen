...
annual.prcp <- df.sites.precip %>%
  group_by(site) %>%
  summarize(map.mm = sum(monthly.precip, na.rm = TRUE))
...
sites.daily.prcp$site = ecosites$property
...
# Tmean
################
pd.tmean <- pd_stack(prism_archive_subset("tmean", "daily", 
                                          minDate = "2019-06-01",
                                          maxDate = "2021-07-31"))
sites.daily.tmean <- as.data.frame(terra::extract(pd.tmean,
                                                  SpatialPoints(eco.coords),
                                                  sp = F))
...
sites.daily.tmean$site = ecosites$property
...
# Tmax
################
pd.tmax <- pd_stack(prism_archive_subset("tmax", "daily", 
                                         minDate = "2019-06-01",
                                         maxDate = "2021-07-31"))
sites.daily.tmax <- as.data.frame(terra::extract(pd.tmax,
                                                 SpatialPoints(eco.coords),
                                                 sp = F))
...
sites.daily.tmax$site = ecosites$property
...
# Tmin
################
pd.tmin <- pd_stack(prism_archive_subset("tmin", "daily", 
                                         minDate = "2019-06-01",
                                         maxDate = "2021-07-31"))

sites.daily.tmin <- as.data.frame(terra::extract(pd.tmin,
                                                 SpatialPoints(eco.coords),
                                                 sp = F))
...
sites.daily.tmin$site = ecosites$property
...
# VPDmax
################
pd.vpdmax <- pd_stack(prism_archive_subset("vpdmax", "daily", 
                                           minDate = "2019-06-01",
                                           maxDate = "2021-07-31"))

sites.daily.vpdmax <- as.data.frame(terra::extract(pd.vpdmax,
                                                   SpatialPoints(eco.coords),
                                                   sp = F))
...
sites.daily.vpdmax$site = ecosites$property
...
# VPDmin
################
pd.vpdmin <- pd_stack(prism_archive_subset("vpdmin", "daily", 
                                           minDate = "2019-06-01",
                                           maxDate = "2021-07-31"))

sites.daily.vpdmin <- as.data.frame(terra::extract(pd.vpdmin,
                                                   SpatialPoints(eco.coords),
                                                   sp = F))
...
sites.daily.vpdmin$site = ecosites$property
...
## Write climate normals .csv
write.csv(daily.clim, "climate_data/TXeco_PRISM_shortTermClimate.csv",
          row.names = FALSE)
